  ...
  if (state.pendingDecision && phase === 'WAITING_FOR_DECISION' && isMyTurn) {
    ...
    return (
      <div className={styles.panel}>
        ...
        <Btn label={`💰 Osta €${p.price}`} onClick={() => cmd('BuyPropertyCommand', { decisionId: dec.decisionId, propertyId: p.propertyId })} variant="primary" />
        <Btn label="🏷 Ohita → huutokauppa" onClick={() => cmd('DeclinePropertyCommand', { decisionId: dec.decisionId, propertyId: p.propertyId })} variant="ghost" />
      </div>
    )
  }
