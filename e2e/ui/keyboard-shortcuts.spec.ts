...
// ─── B / D keys for property decision ─────────────────────────────────────────

test('keyboard: B key buys property in WAITING_FOR_DECISION', async ({ page }) => {
  // Reach WAITING_FOR_DECISION by rolling dice onto an unowned property.
  ...
  try {
    ...
    await injectState(sid, buildPatch({
      ...
      players: seat === 0
        ...
        : [{ cash: 1500, boardIndex: 10 }, { cash: 1500, boardIndex: 0 }],
      turn: { seat, phase: 'WAITING_FOR_ROLL' },
      forcedDice: [1, 2],   // sum=3 → B2 (Kruunuhaka, unowned)
      expectedAfter: {},
    }, snap0))
    ...
    // Decision panel appears after landing on unowned B2
    ...
    await expect(page.getByTestId('action-buy').first()).not.toBeVisible({ timeout: 8000 })
  } finally {
    await deleteSession(sid)
  }
})
...
test('keyboard: ? key opens help modal with keyboard shortcuts', async ({ page }) => {
  const { sid, humanPlayerId, humanPlayerToken } = await createHumanBotSession()
  try {
    await navigateAsHuman(page, sid, humanPlayerId, humanPlayerToken)
    await expect(page.locator('[class*=board]').first()).toBeVisible({ timeout: 8000 })
    ...
    // Help modal shows keyboard shortcut list
    await expect(page.getByText('Välilyönti').first()).toBeVisible({ timeout: 3000 })
    await expect(page.getByText('Heitä nopat').first()).toBeVisible({ timeout: 2000 })
